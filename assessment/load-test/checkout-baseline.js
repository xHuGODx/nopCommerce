...
export default function () {
  ...
  const email = `k6-checkout-${identity}@example.test`;

  group('guest-checkout-baseline', () => {
    ...
    const paymentInfoResponse = groupRequest(
      'save payment info',
      'POST',
    ...
    const paymentInfoJson = parseJson(paymentInfoResponse.body, 'payment info response');
    if (paymentInfoJson.error) {
      fail(`Payment info step failed: ${paymentInfoResponse.body}`);
    }
    ...
    if (confirmJson.error || confirmJson.success !== 1) {
      fail(`Confirm order failed: ${confirmResponse.body}`);
    }

    check(confirmJson, {
      'checkout completed successfully': (body) => body.success === 1,
    });
  });
...
function extractCheckoutAttributes(html) {
  ...
  while ((match = selectRegex.exec(html)) !== null) {
    ...
    const selectedOrFirst = getSelectedOrFirstOptionFromMarkup(match[2], true);
    if (selectedOrFirst) {
